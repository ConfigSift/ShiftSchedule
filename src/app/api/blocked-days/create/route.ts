...
export async function POST(request: NextRequest) {
  ...
  if (!payload.organizationId || !payload.scope || !payload.startDate || !payload.endDate || !payload.reason) {
    return NextResponse.json({ error: 'Missing required fields.' }, { status: 400 });
  }

  if (payload.scope === 'EMPLOYEE' && !payload.userId) {
    return NextResponse.json({ error: 'Employee is required for this block.' }, { status: 400 });
  }
  ...
  if (error || !data) {
    return applySupabaseCookies(
      NextResponse.json({ error: error?.message ?? 'Unable to create blocked day.' }, { status: 400 }),
      response
    );
  }
